...
async function runLoadTest(token, rateLimit, burstLimit, concurrency, durationSeconds) {
    console.log(`  Running: rateLimit=${rateLimit}, burstLimit=${burstLimit}, concurrency=${concurrency}`);
    ...
    return { rateLimit, burstLimit, concurrency, completed, successful, successRate, rps, avgMetrics, rateLimitBlocks };
}

async function main() {
    console.log('\n' + '='.repeat(120));
    console.log('AIClient-2-API COMPLETE STRESS TEST');
    ...
    console.log('\nRateLimit | BurstLimit | Concurrency | Completed | Success | SuccessRate | RPS | CPU% | Blocks');
    console.log('----------|------------|-------------|-----------|---------|-------------|-----|------|-------');
    
    for (const r of results) {
        console.log(`${r.rateLimit.toString().padStart(10)} | ${r.burstLimit.toString().padStart(12)} | ${r.concurrency.toString().padStart(11)} | ${r.completed.toString().padStart(9)} | ${r.successful.toString().padStart(7)} | ${r.successRate.padStart(11)} | ${r.rps.padStart(4)} | ${r.avgMetrics.cpuPercent.padStart(5)} | ${r.rateLimitBlocks.toString().padStart(6)}`);
    }
    ...
    if (optimal) {
        ...
        console.log(`\n✅ System configuration has been updated to optimal values`);
    }
    
    const fs = require('fs');
    fs.mkdirSync('tests/reports', { recursive: true });
    fs.writeFileSync(`tests/reports/complete-stress-report-${Date.now()}.json`, JSON.stringify({
